...
export function MatchFinder() {
  ...
  return (
    <section className="bg-card rounded-3xl ring-1 ring-black/5 shadow-2xl overflow-hidden mb-16">
      <div className="p-8 md:p-12 border-b border-border bg-gradient-to-b from-stone-soft to-card">
        <div className="flex items-center gap-3 mb-4">
          <span className="px-3 py-1 bg-accent/15 text-accent text-xs font-mono rounded-full uppercase tracking-wider flex items-center gap-1.5">
            <Repeat2 className="size-3" /> Match Finder
          </span>
        </div>
        <h2 className="font-display text-3xl md:text-4xl font-extrabold tracking-tight">
          ابحث عن شريك المقايضة المثالي
        </h2>
        <p className="text-muted-foreground mt-3 text-sm leading-relaxed max-w-2xl">
          اكتب ما تملكه وما تبحث عنه — وسنُطابقك مع أصحاب إعلانات يريدون ما لديك ويعرضون ما تريد.
        </p>

        <form
          onSubmit={(e) => { e.preventDefault(); if (have.trim() && want.trim()) m.mutate(); }}
          className="mt-6 grid md:grid-cols-[1fr_auto_1fr_auto] gap-3 items-stretch"
          dir="rtl"
        >
          <label className="flex flex-col gap-1.5">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">أريد أن أُقايض</span>
            <input
              value={have}
      ...
      <div className="p-6 md:p-10 bg-stone-soft/40 min-h-[200px]">
        ...
        {matches.length > 0 && (
          <>
            <h3 className="font-display font-bold text-lg mb-4">
              ...
              <span className="text-muted-foreground text-sm font-normal mr-2">— الأعلى نقاط الأكثر ملاءمة</span>
            </h3>
        ...
        {!m.isSuccess && !m.isPending && !m.isError && (
          <p className="text-center text-sm text-muted-foreground py-12">
            اكتب ما تملكه وما تبحث عنه أعلاه ثم اضغط «ابحث».
          </p>
        )}
      </div>
    </section>
