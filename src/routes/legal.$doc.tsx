...
const DOCS: Record<string, { title: string; html: string }> = {
  terms: {
    title: "شروط الاستخدام — إيكال EQAL",
    html: `
      <h2>1. التعريف</h2>
      <p>إيكال (EQAL) منصة وساطة إلكترونية لتسهيل مقايضة السلع والخدمات بين الأفراد والشركات. المنصة ليست طرفًا في الصفقات ولا تملك المعروضات.</p>
      ...
      <p>المنصة تقدم أدوات تقييم (محرك إيكال) ولكنها لا تضمن نتائج الصفقات. المستخدم يتحمل مخاطر التبادل بشكل كامل.</p>
      ...
      <p>يحق لإيكال تعليق أو إغلاق أي حساب ينتهك هذه الشروط دون إشعار مسبق.</p>
  ...
  privacy: {
    title: "سياسة الخصوصية — إيكال EQAL",
    html: `
  ...
  "barter-agreement": {
    title: "اتفاقية المقايضة — إيكال EQAL",
    html: `
      <h2>1. طبيعة العقد</h2>
      <p>المقايضة عبر إيكال عقد ثنائي مباشر بين الطرفين (أ) و(ب)، تطبق عليه أحكام عقد البيع في النظام السعودي وأحكام الفقه الإسلامي.</p>
      <h2>2. أركان العقد</h2>
      <ul>
      ...
      <h2>5. النزاعات</h2>
      <p>أي نزاع يُحل أولًا عبر نظام النزاعات في إيكال (تحكيم داخلي)، ثم القضاء السعودي إن لم يُحسم.</p>
      <h2>6. الإلغاء</h2>
      <p>يحق إلغاء الصفقة قبل قبولها. بعد القبول، يلزم اتفاق الطرفين أو حكم تحكيم.</p>
    `,
  },
  "anti-riba": {
    title: "سياسة مكافحة الربا — إيكال EQAL",
    html: `
      <h2>المبدأ</h2>
      <p>إيكال منصة شرعية تلتزم بأحكام البيع والصرف في الفقه الإسلامي، وتمنع تقنيًا أي معاملة تقع في الربا (فضل/نسيئة).</p>
...
export const Route = createFileRoute("/legal/$doc")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: loaderData?.doc.title ?? "وثيقة قانونية — إيكال" },
      { name: "description", content: `${loaderData?.doc.title ?? "وثيقة قانونية"} — منصة إيكال للمقايضة العادلة.` },
    ],
  }),
  component: LegalDoc,
  notFoundComponent: () => (
    ...
    <div dir="rtl" className="min-h-screen bg-background font-body">
      <header className="border-b border-border">
        <div className="max-w-3xl mx-auto px-6 py-5 flex items-center justify-between">
          <Link to="/" className="font-display text-2xl font-extrabold text-primary">إيكال EQAL</Link>
